...
export default function AdminJAFPage() {
  ...
  // Helper function to map status to tab value
          ...
          <TabsContent key={tab} value={tab}>
            <Card className="p-6">
              <Table>
                ...
                <TableBody>
                  {filteredJAFs
                    .filter(jaf => getTabValue(jaf.status) === tab)
                    .map((jaf) => (
                      <TableRow key={jaf.id}>
                        <TableCell>
              ...
              {filteredJAFs.filter(jaf => getTabValue(jaf.status) === tab).length === 0 && (
                <div className="text-center py-4 text-muted-foreground">
                  No JAFs found in this category
                </div>
              )}
            </Card>
          </TabsContent>
